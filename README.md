...
python3 harness/calibrate.py \
    ...
    --engine "vllm 0.11.0" --engine-config "chunked-prefill on, max_num_seqs 256"
...
같은 하드웨어도 어떻게 최적화했느냐로 답이 달라진다. 다만 최적화는 두 부류로 갈린다.

| 부류 | 예 | 처리 |
|---|---|---|
| **계수를 움직임** | 가중치·KV 양자화, prefix cache, CUDA graph, 커널 튜닝 | η·스텝 오버헤드가 흡수 — 캘리브레이션이 자동 반영 |
| **식을 바꿈** | 추측 디코딩, 프리필·디코드 분리(PD) | 「서빙 최적화」 패널에서 직접 지정 |

- **추측 디코딩** — 스텝당 accept 개 토큰이 나오고 드래프트 비용만큼 스텝이 길어진다.
  수용 길이를 상수로 두므로 **큰 배치에서는 낙관적**이다(실제로는 배치가 커지면 떨어진다).
- **PD 분리** — 두 풀이 독립이라 디코드가 프리필에 시간을 뺏기지 않는다. 처리량은
  `min(프리필 풀, 디코드 풀)`, KV 는 디코드 풀에만 쌓이고 가중치는 양쪽에 중복 상주한다.
  풀 간 KV 전송 비용은 아직 모델에 없다.

  **고정된 GPU 총량에서는 PD 가 처리량을 깎는 쪽으로 나오는 게 정상이다.** H100 8장·70B·RAG
  기준으로 통합 138 동시 요청 vs PD 4+4 는 34 인데, 대신 사용자당 속도가 20 → 64 tok/s 로
  뛴다. PD 는 같은 총량에서 처리량을 지연 예측성과 맞바꾸는 것이고, 진짜 이득인 "두 풀을
  각각 다른 하드웨어로 독립 증설" 은 이 모델이 아직 표현하지 못한다.

그리고 캘리브레이션 계수는 **"이 하드웨어"가 아니라 "이 하드웨어 + 이 엔진 설정"** 에 붙는다.
`--engine` / `--engine-config` 로 기록하면 계산기가 등급 A 옆에 그 설정을 같이 띄운다.
